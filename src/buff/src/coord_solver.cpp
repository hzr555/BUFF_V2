 ...
 */
EulerAngle CoordSolver::getAngleCompensation(Eigen::Vector3d &xyz_camera, Eigen::Matrix3d &rotmat_imu)
{
    ...
    cv::Point dest_point;
    cv::Point center(640, 360);
    dest_point.x = xyz_pixel[0];
    dest_point.y = xyz_pixel[1];

    angle_offset = GetAngle(center, dest_point);
...
{
    double angle_x_bias_ = 12, angle_y_bias_ = 12;
...
EulerAngle CoordSolver::GetAngle(cv::Point center,cv::Point dest_point){
	return calculate_angle(cv::Point(0, 0), dest_point - center, CAMERA_FOCUS_);
}

// }  //namespace coord
// #include <rclcpp_components/register_node_macro.hpp>
// RCLCPP_COMPONENTS_REGISTER_NODE(coord::CoordSolver)
